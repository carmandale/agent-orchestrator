...
- Using `unlinkSync` or `rm` to clean up state files — use `trashFile()` which calls the macOS `trash` CLI for Finder-recoverable cleanup
...
7. **Trash, don't delete** — state file cleanup uses the macOS `trash` CLI (proper Finder API with "Put Back" support), falling back to `unlinkSync` when unavailable. See `trashFile()` in `packages/cli/src/lib/web-dir.ts`
